...
use agent_llm_core::{
    ...
    settings::{default_admin_base_url, default_db_path},
...
struct AuthAddArgs {
    ...
    #[arg(
        long,
        help = "One of: api_key, openai_session, anthropic_session, none"
    )]
    auth_mode: String,
    #[arg(
        long,
...
fn build_local_env(db: &Database, project_name: &str) -> Result<String> {
    ...
    let kimi = db.provider(ProviderKind::Kimi)?;
    let lmstudio = db.provider(ProviderKind::LmStudio)?;
    let responses_base = format!("{}/v1", default_admin_base_url());
...
\n\
# Kimi (Moonshot) — Anthropic-protocol passthrough.\n\
# For a Claude Code session on Kimi, set ANTHROPIC_BASE_URL={kimi_url} instead.\n\
KIMI_BASE_URL={kimi_url}\n\
KIMI_API_KEY={key}\n\
\n\
...
\n\
# Codex CLI and other Responses-API clients: point base_url at\n\
# {responses_base} and request model \"provider/model\", e.g. \"lmstudio/qwen/qwen3.6-35b-a3b\".\n\
",
        ...
        kimi_url = kimi.local_base_url,
        lmstudio_url = lmstudio.local_base_url,
...
KIMI_API_KEY=\n\
\n\
LMSTUDIO_BASE_URL={lmstudio_url}/v1\n\
LMSTUDIO_API_KEY=\n\
",
        ...
        kimi_url = kimi.upstream_base_url,
        lmstudio_url = lmstudio.upstream_base_url,
    ))
}
...
fn print_auth_modes() {
    ...
    println!(
        "  none               No credentials (lmstudio only; a default profile ships preconfigured)"
    );
    println!();
    println!("Examples:");
    println!(
    ...
    println!("  agent-llm auth verify --provider openai --profile codex");
}
...
fn verify_auth_profile(db: &Database, args: AuthVerifyArgs) -> Result<()> {
    ...
    let body = response.json::<Value>().unwrap_or_else(|_| Value::Null);
...
fn collect_models_preview(provider: ProviderKind, body: &Value) -> Vec<String> {
    let items = match provider {
        ...
        | ProviderKind::Kimi
        | ProviderKind::LmStudio => body.get("data").and_then(Value::as_array),
